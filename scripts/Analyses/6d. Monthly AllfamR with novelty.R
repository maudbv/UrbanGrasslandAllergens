# Analyse trends in mean monthly number of unique Allergen Families

glms.allfam.month <- (function(exclude.absences = TRUE,
                              show.plots = TRUE){
  ...
  glms.table$type <- factor(glms.table$type,
                            levels = c("allfam.mean","log.allfam.wtd.mean"))
  glms.table$level <- "AllFam"
...
# Export result table: ####
write.csv(glms.allfam.month$glms.table,
          "results/Monthly allfam richness models table.csv")
write.csv(glms.allfam.month$glms.best.pred,
          "results/Monthly allfam richness best predictor table.csv")
