# Analyse trends in mean monthly allergenic flowering over the year
### unweighted cumulated molecules: only Neophyte significant

glms.AR.month <- (function(exclude.absences = TRUE,
                              show.plots = TRUE){
  ...
  # Export result table: ####
  write.csv(glms.table,
            "results/Monthly AR models table.csv")
  write.csv(glms.best.pred,
            "results/Monthly AR  best predictor table.csv")
